...
class EvalMinimumSnapNode {
public:

    EvalMinimumSnapNode(ros::NodeHandle & nh, std::string const& waypoint_topic, std::string const& local_goal_topic, std::string const& samples_topic) {
        waypoints_sub = nh.subscribe(waypoint_topic, 10, &EvalMinimumSnapNode::OnWaypoints, this);
...
int main(int argc, char* argv[]) {
    std::cout << "Initializing eval minimum snap node" << std::endl;

    ros::init(argc, argv, "EvalMinimumSnapNode");
    ros::NodeHandle nh;

    EvalMinimumSnapNode eval_minimum_snap_node(nh, "/waypoint_list", "/goal_passthrough", "/poly_samples");
